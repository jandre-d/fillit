...
/*   Updated: 2019/01/28 09:44:51 by tde-jong      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
static t_gnl_list	*get_fd_list(ssize_t fd, t_gnl_list **firstlist)
{
	...
	if ((new = (t_gnl_list*)ft_memalloc(sizeof(t_gnl_list))) == NULL)
		return (NULL);
	if ((new->str = (char*)ft_memalloc(BUFF_SIZE)) == NULL)
		return (NULL);
	new->fd = fd;
	if (list == NULL)
		return ((*firstlist = new));
	else
		...
		ssize_t add_len)
{
	...
	if ((new_line = (char*)ft_memalloc(*len + add_len + (ssize_t)1)) ==
			NULL)
		return (NULL);
...
int					get_next_line(const int fd, char **line)
{
	...
	if (fd < 0 || line == NULL || BUFF_SIZE < 1)
		return (-1);
	*line = ft_strnew(0);
	if ((item = get_fd_list(fd, &firstlist)) == NULL)
		return (-1);
	if ((ret = get_from_item(line, item, &len)) != 0)
		return (ret);
	while ((item->str_len = read(fd, item->str, BUFF_SIZE)) != 0)
	{
		if ((ret = get_from_item(line, item, &len)) != 0)
		{
			if (ret == -1)
